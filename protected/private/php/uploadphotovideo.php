        ...
        function upload($filePath, $fileName, $fileIndex, $uploadedFilesQuantity){
            ...
            if(move_uploaded_file($filePath, $path))  {
                ...
                if(isset($_POST["submitform"]) || isset($_POST["submit"]) || isset($_POST["ps"]))    {
                    ...
                    $descriptionHTML = str_replace("value_n", $GLOBALS["filesQuantity"], str_replace("value_key", $key, $descriptionHTML));
                    $voiceHTML = str_replace("value_n", $GLOBALS["filesQuantity"], str_replace("value_key", $key, $voiceHTML));
                    if(isset($_POST["ps"]))    {
                        echo(str_replace("</h1>", "</h1><div style=\"border:2px solid #00ff00;\">upload completed<br><a target=\"_blank\" href=\"../?" . $GLOBALS["filesQuantity"] . "\">view upload</a></div>", file_get_contents($_SERVER["DOCUMENT_ROOT"] . "/ps/index.php")));
